...
class DCVC_net(nn.Module):
    ...
    def encode_decode(self, ref_frame, input_image, output_path):
        ...
        encoded = self.encode(ref_frame, input_image, output_path, encoder_type)
        decoded = self.decode(ref_frame, output_path, encoder_type)
        encoded['recon_image'] = decoded
        return encoded

    def encode(self, ref_frame, input_image, output_path, encoder_type):
        from ..utils.stream_helper import encode_p
        N, C, H, W = ref_frame.size()
        compressed = self.compress(ref_frame, input_image, encoder_type)
    ...
    def decode(self, ref_frame, input_path, encoder_type):
        ...
        return self.decompress(ref_frame, mv_y_string, mv_z_string,
                               y_string, z_string, height, width, encoder_type)
    def compress_ar(self, y, kernel_size, context_prediction, params, entropy_parameters, encoder):
        assert encoder in ['rlvc', 'dcvc']
        kernel_size = 5
        ...
        if encoder == 'rlvc':
            bitout = arithmeticcoding.BitOutputStream(open(f'tmp_{MY_HEX}.bin', 'wb'))
        ...
        for h in range(height):
            for w in range(width):
                ...
                if flip_probability <= flip_threshold:
                    y_crop = means_hat
                if encoder == 'rlvc':
                    scales_hat = scales_hat.clamp(1e-5, 1e10)
                    scales_hat = torch.log2(scales_hat)

                # ONLY FOR TESTING 100% flip change
                # if encoder == 'rlvc':
                #     scales_hat = -6 * torch.ones_like(scales_hat)
                # else:
                #     scales_hat = 1e-5 * torch.ones_like(scales_hat)
                
                y_crop_q = torch.round(y_crop - means_hat)
                y_hat[0, :, h + padding, w + padding] = (y_crop_q + means_hat)[0, :, 0, 0]
                if encoder == 'rlvc':
                    for j in range(scales_hat.shape[1]):
                        freq = arithmeticcoding.logFrequencyTable_exp(50, scales_hat[0,j,0,0], 101)
                        enc.write(freq, y_crop_q[0,j,0,0] + 50)
        ...
        if encoder == 'rlvc':
            enc.finish()
            bitout.close()
            with open(f'tmp_{MY_HEX}.bin', 'rb') as f:
                y_string = f.read()
        else:
    ...
    def decompress_ar(self, y_string, channel, height, width, downsample, kernel_size,
                      context_prediction, params, entropy_parameters, encoder):
        assert encoder in ['rlvc', 'dcvc']
        ...
        if encoder == 'rlvc':
            with open(f'tmp_out_{MY_HEX}.bin', 'wb') as fp:
                fp.write(y_string)
            bitin = arithmeticcoding.BitInputStream(open(f'tmp_out_{MY_HEX}.bin', 'rb'))
        ...
        for h in range(y_height):
            for w in range(y_width):
                ...
                if encoder == 'rlvc':
                    rv = torch.zeros_like(means_hat)
                    for j in range(means_hat.shape[1]):
                        freq = arithmeticcoding.logFrequencyTable_exp(50, scales_hat[0,j,0,0], 101)
                        rv[0,j,0,0] = dec.read(freq) - 50
                else:
    ...
    def compress(self, referframe, input_image, encoder_type):
        ...
        mv_y_string, mv_y_hat = self.compress_ar(mvfeature, 5, self.auto_regressive_mv,
                                                 params_mv, self.entropy_parameters_mv, encoder_type)
        ...
        y_string, y_hat = self.compress_ar(feature, 5, self.auto_regressive,
                                           torch.cat((temporal_prior_params, params), dim=1), self.entropy_parameters, encoder_type)
    ...
    def decompress(self, referframe, mv_y_string, mv_z_string, y_string, z_string, height, width, encoder_type):
        device = next(self.parameters()).device
        ...
        mv_y_hat = self.decompress_ar(mv_y_string, self.out_channel_mv, height, width, 16, 5,
                                      ...
                                      self.entropy_parameters_mv, encoder_type)
        ...
        y_hat = self.decompress_ar(y_string, self.out_channel_M, height, width, 16, 5,
                                   ...
                                   self.entropy_parameters, encoder_type)
        recon_image_feature = self.contextualDecoder_part1(y_hat)
        recon_image = self.contextualDecoder_part2(torch.cat((recon_image_feature, context) , dim=1))
