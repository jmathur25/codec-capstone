...
class DCVC_net(nn.Module):
    ...
    def forward(self, referframe, input_image, compress_type=True):
        ...
        if compress_type == 'full':
            ...
            delta_mvfeature_ar = self.mvfeatureDeltaAutogressive(torch.round(mvfeature))
            delta_mvfeature = 2 * self.mvfeatureDelta(
                torch.cat((delta_mvfeaturePrior, delta_mvfeature_ar), dim=1)
            )
            # quant_mv = torch.round(mvfeature)
            quant_mv = torch.round(mvfeature / delta_mvfeature) * delta_mvfeature
        else:
            delta_mvfeaturePrior = self.mvfeatureDeltaPrior(compressed_z_mv)
        ...
        if compress_type == 'full':
            ...
            delta_feature_ar = self.featureDeltaAutogressive(torch.round(feature_renorm))
            delta_feature = 2 * self.featureDelta(
                torch.cat((delta_featurePrior, delta_feature_ar, delta_mvfeature), dim=1)
            )
            compressed_y_renorm =  torch.round(feature_renorm / delta_feature) * delta_feature
        else:
            delta_featurePrior = self.featureDeltaPrior(
                torch.cat((compressed_z, compressed_z_mv), dim=1),
            )
