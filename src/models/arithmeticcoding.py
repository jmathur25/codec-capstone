...
class ArithmeticCoderBase(object):
    ...
    def update(self, freqs, symbol):
        ...
        span = high - low + 1
        if not (self.minimum_range <= span <= self.full_range):
        ...
        # Update range
        newlow = low + symlow * span // total
        newhigh = low + symhigh * span // total - 1
        self.low = int(newlow)
        ...
        while ((self.low ^ self.high) & self.half_range) == 0:
            ...
            # stops infinite loop if low=high. Will allow encoder to reset.
        ...
        while (self.low & ~self.high & self.quarter_range) != 0:
            ...
            # gets a leading 1 and last bit of 1
...
class ArithmeticDecoder(ArithmeticCoderBase):
    ...
    def underflow(self):
        # keeps top bit, skips 2nd bit, keeps the rest.
        # also fills in the last bit with a new code bit.
        self.code = (
    ...
    def get(self, symbol):
        ...
        if symbol == self.EOF:
            ...
            c2 = 0.5 * (
                1
                + scipy.special.erf(
                    (symbol + 0.5 - self.mu_val)
                    / ((self.sigma_val + self.TINY) * 2**0.5)
                )
            )
            c1 = 0.5 * (
                1
...
class logFrequencyTable(FrequencyTable):
    ...
    def get(self, symbol):
        ...
        if symbol == self.EOF:
            ...
            c2 = scipy.special.expit(
                np.multiply(
                    (symbol + 0.5 - self.mu_val), (self.sigma_val**2 + self.TINY)
                )
            )
            c1 = scipy.special.expit(
                np.multiply(
                    (symbol - 0.5 - self.mu_val), (self.sigma_val**2 + self.TINY)
                )
            )
    ...
    def get_low(self, symbol):
        self._check_symbol(symbol)
        c = scipy.special.expit(
            np.multiply(
                (symbol - 1 + 0.5 - self.mu_val), (self.sigma_val**2 + self.TINY)
            )
        )
    ...
    def get_high(self, symbol):
        self._check_symbol(symbol)
        c = scipy.special.expit(
            np.multiply((symbol + 0.5 - self.mu_val), (self.sigma_val**2 + self.TINY))
        )
...
class logFrequencyTable_exp(FrequencyTable):
    ...
    def get(self, symbol):
        ...
        if symbol == self.EOF:
            ...
            c2 = scipy.special.expit(
                np.multiply(
                    (symbol + 0.5 - self.mu_val), (np.exp(-self.sigma_val) + self.TINY)
                )
            )
            c1 = scipy.special.expit(
                np.multiply(
    ...
    def get_low(self, symbol):
        ...
        c = scipy.special.expit(
            np.multiply(
                (symbol - 1 + 0.5 - self.mu_val), (np.exp(-self.sigma_val) + self.TINY)
            )
        )
        # c = int(math.floor(c * self.mul_factor))
        c = int(math.floor(c * self.mul_factor) + symbol)
