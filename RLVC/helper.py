...
def configure_decoder(args):
    ...
    return activation, GOP_size, GOP_num, path, path_com, path_bin, path_lat
...
def encode_I(args, frame_index, I_level, path, path_com, path_bin):
    ...
    if args.mode == "PSNR":
        ...
        os.system(
            args.python_path + " " + args.CA_model_path + "/encode.py --model_type 1 "
            "--input_path "
            + path
            + "f"
            + str(frame_index).zfill(3)
        ...
        os.system(
            ...
            + " "
            + args.CA_model_path
            + "/decode.py --compressed_file_path "
            + path_bin
            + "f"
            + str(frame_index).zfill(3)
            + ".bin"
