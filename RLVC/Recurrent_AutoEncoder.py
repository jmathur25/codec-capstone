...
parser.add_argument(
    "--l", type=int, default=1024, choices=[8, 16, 32, 64, 256, 512, 1024, 2048]
)
...
# Placeholder
Y0_com_tensor = tf.placeholder(
    tf.float32, [batch_size, Height, Width, Channel]
)  # reference frame
Y1_raw_tensor = tf.placeholder(
    tf.float32, [batch_size, Height, Width, Channel]
)  # raw frame to compress

hidden_states = tf.placeholder(
    tf.float32, [8, batch_size, Height // 4, Width // 4, args.N]
)  # hidden states in RAE
...
(
    ...
    c_dec_res,
    h_dec_res,
) = tf.split(hidden_states, 8, axis=0)

RPM_flag = tf.placeholder(tf.bool, [])  # use RPM (=1) or bottleneck (=0)

# motion estimation
with tf.variable_scope("flow_motion"):
    motion_tensor, _, _, _, _, _ = motion.optical_flow(
        Y0_com_tensor, Y1_raw_tensor, batch_size, Height, Width
    )

# RAE encoder for motion
motion_latent, c_enc_mv_out, h_enc_mv_out = CNN_recurrent.MV_analysis(
    motion_tensor,
    num_filters=args.N,
    out_filters=args.M,
    Height=Height,
...
motion_latent_decom = entropy_bottleneck.decompress(
    tf.expand_dims(string, 0), [Height // 16, Width // 16, args.M], channels=args.M
)
motion_latent_hat = tf.cond(
    RPM_flag, lambda: tf.round(motion_latent), lambda: motion_latent_decom
)

# RAE decoder for motion
motion_hat, c_dec_mv_out, h_dec_mv_out = CNN_recurrent.MV_synthesis(
    motion_latent_hat,
    num_filters=args.N,
    Height=Height,
    Width=Width,
...
res_latent, c_enc_res_out, h_enc_res_out = CNN_recurrent.Res_analysis(
    Res,
    num_filters=args.N,
    out_filters=args.M,
    Height=Height,
...
res_latent_decom = entropy_bottleneck2.decompress(
    tf.expand_dims(string2, 0), [Height // 16, Width // 16, args.M], channels=args.M
)
res_latent_hat = tf.cond(
    RPM_flag, lambda: tf.round(res_latent), lambda: res_latent_decom
)

# RAE decoder for residual
res_hat, c_dec_res_out, h_dec_res_out = CNN_recurrent.Res_synthesis(
    res_latent_hat,
    num_filters=args.N,
    Height=Height,
    Width=Width,
...
if args.metric == "PSNR":
    ...
    quality_tensor = tf.math.reduce_mean(
        tf.image.ssim_multiscale(Y1_decoded, Y1_raw_tensor, max_val=1)
    )
...
for g in range(GOP_num):
    ...
    for f in range(args.f_P):
        ...
        if f % 6 == 0:
            h_state = np.zeros(
                [8, batch_size, Height // 4, Width // 4, args.N], dtype=np.float
            )
        ...
        (
            ...
            feed_dict={
                Y0_com_tensor: F0_com / 255.0,
                Y1_raw_tensor: F1_raw / 255.0,
...
for f in range(rest_frame_num):
    ...
    # run RAE
    F0_com, string_MV, string_Res, quality, h_state, latent_mv, latent_res = sess.run(
        [
            Y1_decoded,
            string,
            string2,
