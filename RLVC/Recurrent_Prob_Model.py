...
# Placeholder
prior_tensor = tf.placeholder(
    tf.float32, [batch_size, Height // 16, Width // 16, args.M]
)  # previous latent
latent_tensor = tf.placeholder(
    tf.float32, [batch_size, Height // 16, Width // 16, args.M]
)  # latent to compress

hidden_states = tf.placeholder(
    tf.float32, [2, batch_size, Height // 16, Width // 16, args.N]
)  # hidden states in RPM

c_prob, h_prob = tf.split(hidden_states, 2, axis=0)

# RPM network
prob_latent, c_prob_out, h_prob_out = CNN_recurrent.rec_prob(
    prior_tensor, args.N, Height, Width, c_prob[0], h_prob[0]
)
...
for lat in latents:
    ...
    for g in range(GOP_num):
        ...
        if args.f_P >= 2:
            ...
            # init state
            h_state = np.zeros(
                [2, batch_size, Height // 16, Width // 16, args.N], dtype=np.float
            )
            ...
            for f in range(args.f_P - 1):
                ...
                # run RPM
                bits_estimation, sigma_value, mu_value, h_state = sess.run(
                    [bits_est, sigma, mu, hidden_states_out],
                    feed_dict={
                        prior_tensor: prior_value,
                        latent_tensor: latent_value,
                        hidden_states: h_state,
                    },
                )

                if args.entropy_coding:
                    bits_value = helper.entropy_coding(
                        frame_index, lat, path_bin, latent_value, sigma_value, mu_value
                    )
