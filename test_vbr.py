...
from src.models.DCVC_net_vbr import DCVC_net
...
def parse_args():
    ...
    parser.add_argument(
        "--write_stream", type=str2bool, nargs="?", const=True, default=False
    )
    parser.add_argument(
        "--write_recon_frame", type=str2bool, nargs="?", const=True, default=False
    )
...
def read_frame_to_torch(path):
    ...
    x1 = (x - crop_size) // 2
    y1 = (y - crop_size) // 2
    input_image = input_image.crop((x1, y1, x1 + crop_size, y1 + crop_size))
...
def encode_one(args_dict, device):
    i_frame_load_checkpoint = torch.load(
        args_dict["i_frame_model_path"], map_location=torch.device("cpu")
    )
    i_frame_net = (
        architectures[args_dict["i_frame_model_name"]]
        .from_state_dict(i_frame_load_checkpoint)
        .eval()
    )

    video_net = DCVC_net()
    load_checkpoint = torch.load(
        args_dict["model_path"], map_location=torch.device("cpu")
    )
    ...
    recon_bin_folder = os.path.join(
        args_dict["recon_bin_path"],
        sub_dir_name,
    ...
    with torch.no_grad():
        for frame_idx in range(frame_num):
            ...
            if args_dict["write_recon_frame"]:
                write_torch_frame(
                    ref_frame.squeeze(),
                    os.path.join(recon_bin_folder, f"recon_frame_{frame_idx}.png"),
                )
    ...
    log_result["ave_i_frame_bpp"] = (
        cur_all_i_frame_bit / cur_i_frame_num / frame_pixel_num
    )
    ...
    if cur_p_frame_num > 0:
        ...
        log_result["ave_p_frame_bpp_mv_y"] = (
            cur_all_p_frame_bit_mv_y / total_p_pixel_num
        )
        log_result["ave_p_frame_bpp_mv_z"] = (
            cur_all_p_frame_bit_mv_z / total_p_pixel_num
        )
    ...
    log_result["ave_all_frame_quality"] = (
        cur_all_i_frame_quality + cur_all_p_frame_quality
    ) / frame_num
    return log_result
...
def filter_dict(result):
    keys = [
        ...
        "ave_i_frame_quality",
        "ave_p_frame_bpp",
        "ave_p_frame_bpp_mv_y",
        "ave_p_frame_bpp_mv_z",
        "ave_p_frame_bpp_y",
        "ave_p_frame_bpp_z",
        "ave_p_frame_quality",
        "ave_all_frame_bpp",
        "ave_all_frame_quality",
    ]
...
def main():
    ...
    for ds_name in config:
        for seq_name in config[ds_name]["sequences"]:
            ...
            for model_idx in range(len(args.model_path)):
                ...
                obj = threadpool_executor.submit(worker, args.cuda, cur_dict)
                objs.append(obj)
    ...
    for ds_name in config:
        ...
        for seq in config[ds_name]["sequences"]:
            ...
            for model_idx in range(len(args.model_path)):
                ...
                for res in results:
                    if (
                        res["name"].startswith(ckpt)
                        and ds_name == res["ds_name"]
    ...
    print("Test finished")
    print(
        f"Tested {count_models} models on {count_frames} frames from {count_sequences} sequences"
    )
